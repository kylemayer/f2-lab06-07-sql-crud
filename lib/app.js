...
app.get('/ducks', async(req, res) => {
  try {
    const data = await client.query('SELECT * from ducks');
...
app.get('/ducks/:id', async(req, res) => {
  try {
    const data = await client.query('SELECT * from ducks where id=$1', [req.params.id]);
...
app.post('/ducks', async(req, res) => {
  try {
    ...
    VALUES ($1, $2, $3, $4, 1)
    RETURNING *`, [req.body.name, req.body.mass_oz, req.body.body_size, req.body.feet_color]);
...
app.put('/ducks/:id', async(req, res) => {
  try {
    ...
    WHERE id=$5
    RETURNING *
    `, [req.body.name, req.body.mass_oz, req.body.body_size, req.body.feet_color, req.params.id]);
