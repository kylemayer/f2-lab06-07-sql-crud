...
app.get('/ducks', async(req, res) => {
  try {
    const data = await client.query(`
    SELECT 	dk.id, dk.name, dk.complexity, c.name as category, dk.feet_color, dk.owner_id
    FROM 	  ducks as dk
    JOIN 	  categories as c
    ON 	 	  dk.category_id = c.id;
  `);
...
app.get('/ducks/:id', async(req, res) => {
  try {
    const data = await client.query(`
    SELECT 	dk.id, dk.name, dk.complexity, c.name as category, dk.owner_id
    FROM 	  ducks as dk
    JOIN 	  categories as c
    ON 	 	  dk.category_id = c.id;
    WHERE   dk.id = $1;
